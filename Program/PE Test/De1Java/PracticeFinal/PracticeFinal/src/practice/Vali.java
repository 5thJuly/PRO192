...
public class Vali {
    ...
    public int f1(List<Book> a) {
        ...
        for (Book o : a) {
            ...
            for (int i = 0; i < code.length(); i++) {
                ...
                if (Character.isLetter/*kí tự chữ */(code.charAt(i))) {
                    check2 = true;
                }
            }
    ...
    //nhap index you want to change code
    //input code: MLN1051309hn
    //output code:MLN2162419hn
    public String f2(List<Book> a, int index) {
        String result = "" ;
        String code = a.get(index).getCode() ; // lấy code ở phần tử index
        for (int i = 0; i < code.length(); i++) {
            //các số từ 0 đến 8 : (47->56) trong bảng mã ASCII
            // '0' <=> 47 - '8'<=> 56
            if(code.charAt(i)>='0' && code.charAt(i)<='8') {
                // //ép số // cách 1
                // int number = Integer.parseInt(code.charAt(i)+ "")+1;// tăng 1 đơn vị nên phải +1
    ...
    public void f3(List<Book> a) {
        for (Book o : a) {
            ...
            o.setCode(result); // thay đổi code = source code
        ...
        Collections.sort(a, new Comparator<Book>() {
            ...
            public int compare(Book o1, Book o2) {
                if(o1.getPrice() > o2.getPrice()) {
                    ...
                    // sap xep ten theo thu tu tang dan alphabet
    ...
    public void f10(List<Book> a) {
        // B1 : tạo list và add tất cả book truyện dài vào list đó
        // sort list tăng dần theo price
        // 
    ...
    public String f11(List<Book> a, int index) {
        String result = "";
        String code = a.get(index).getCode() ;
        for (int i = 0; i < code.length(); i++) {
            if(Character.isLetter(code.charAt(i)) && code.charAt(i) != 'z' && code.charAt(i) != 'Z') {
                char c = (char) ((int) code.charAt(i) + 1) ;
                result = result + c ;
            }
            else {
